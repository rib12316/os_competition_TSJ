...
**一句话（全量更新）**：F2 在全量 115 任务下 **success 红线通过（+2.6pp，不伤精度）、上下文 -50%**，
机制完全成立；但 **gpt2 单 worker 压缩太慢（总 24 分钟），在并发下拖累延迟（+16%）**。下一步换
BERT 压缩器 + worker 池即可把延迟这块也赢回来——success 这块（赛题最硬指标）已经赢了。
